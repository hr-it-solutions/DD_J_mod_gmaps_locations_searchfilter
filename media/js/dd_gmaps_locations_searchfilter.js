        ...
        if(address.val !== '') {
            ...
            geocoder.geocode( { 'address': address.val}, function(results, status) {
                if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    jQuery('#locationLatLng').val = results[0].geometry.location.lat() + ',' + results[0].geometry.location.lng();
                }
            });
        }
    }, 300);
...
var initAutoCompleteListener = function initAutoCompleteListener() {
    ...
    var input = jQuery('#dd_input_location_search');
    var options = {types:[]};
    ...
    google.maps.event.addListener(autocomplete, 'place_changed', function() {
        ...
        jQuery('#dd_input_geolocate').value = '';
...
function showPosition(position) {
    ...
    jQuery('#locationLatLng').val = position.coords.latitude + ',' + position.coords.longitude;
    jQuery('#dd_gmaps_locations_searchfilter_form').submit();
}
